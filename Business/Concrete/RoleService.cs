...
namespace Business.Concrete
{
	public class RoleService : IRoleService
	{
		...
		public async Task<IResponse> GetAssignedRoles(string userid)
		{
			...
			List<string> userroles = _userManager.GetRolesAsync(user).Result as List<string> ?? new List<string>();
			List<AssignRoleDTO> AssignedRole = new List<AssignRoleDTO>();
			foreach (var role in roles)
			{
				AssignRoleDTO r = new AssignRoleDTO();
				r.UserId = user.Id;
				r.RoleId = role.Id;
			...
			return new DataResponse<List<AssignRoleDTO>>(AssignedRole, 200);
		}
		...
		public async Task<IResponse> RoleAssign(List<AssignRoleDTO> models)
		{
